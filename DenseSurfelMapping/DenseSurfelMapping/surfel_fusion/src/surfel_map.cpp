...
HeightCloudFilterOptions::HeightCloudFilterOptions():
crop_min(
...
crop_max(
    ...
    std::numeric_limits<float>::infinity(),
    std::numeric_limits<float>::infinity()),
trajectory_keep_radius(2.5f),
density_voxel_size(0.20f),
density_min_neighbors(4),
remove_low_height_outliers(true),
low_height_percentile(0.02f),
low_height_margin(0.05f)
{
}
...
rgb_inactive_pcd (new RgbPointCloud)
{
    ...
    nh.param("offline_mode", offline_mode, false);
    if(!offline_mode)
    {
...
rgb_inactive_pcd(new RgbPointCloud)
{
    ...
    far_dist = fuse_far;
    near_dist = fuse_near;
    drift_free_poses = drift_free_pose_count;
    ...
    printf("Have the following offline settings: \n");
    printf("camera matrix: \n");
    cout << camera_matrix << endl;
    printf("fuse the distence between %4f m and %4f m.\n", near_dist, far_dist);
...
static cv::Mat depth_to_gray_image(const cv::Mat &depth, float near_dist, float far_dist)
{
    cv::Mat gray(depth.rows, depth.cols, CV_8UC1, cv::Scalar(0));
    const float range = std::max(far_dist - near_dist, 1e-6f);
    for(int y = 0; y < depth.rows; y++)
    {
        ...
        for(int x = 0; x < depth.cols; x++)
        {
            ...
            if(std::isfinite(d) && d >= near_dist && d <= far_dist)
            {
                const float normalized = (far_dist - d) / range;
                gray_row[x] = static_cast<uchar>(std::max(0.0f, std::min(255.0f, normalized * 255.0f)));
            }
        }
    }
    return gray;
...
void SurfelMap::process_offline_frame(const ros::Time &stamp, cv::Mat &depth, Eigen::Matrix4f &pose)
{
    if(depth.empty())
        return;
    if(depth.type() != CV_32FC1)
    {
        ROS_ERROR("offline depth must be CV_32FC1 meters");
        return;
    }
    if(depth.cols != cam_width || depth.rows != cam_height)
    {
        ROS_ERROR("offline depth size %dx%d does not match camera params %dx%d",
                  depth.cols, depth.rows, cam_width, cam_height);
        return;
    }

    PoseElement this_pose_element;
    int this_pose_index = poses_database.size();
    Eigen::Matrix4d pose_d = pose.cast<double>();
    pose_eigen2ros(pose_d, this_pose_element.cam_pose);
    this_pose_element.loop_pose = this_pose_element.cam_pose;
    this_pose_element.cam_stamp = stamp;
    if(this_pose_index > 0)
    {
        int relative_index = this_pose_index - 1;
    ...
    cv::Mat gray = depth_to_gray_image(depth, near_dist, far_dist);
    move_add_surfels(this_pose_index);
    fuse_map(gray, depth, pose, this_pose_index);
    move_all_surfels();
}
...
static int height_axis_index(const string &height_axis)
{
    if(height_axis == "x")
        return 0;
    if(height_axis == "y")
...
static void jet_color(float value, uint8_t &r, uint8_t &g, uint8_t &b)
{
    value = std::max(0.0f, std::min(1.0f, value));
    float rf = std::min(std::max(1.5f - std::fabs(4.0f * value - 3.0f), 0.0f), 1.0f);
    float gf = std::min(std::max(1.5f - std::fabs(4.0f * value - 2.0f), 0.0f), 1.0f);
    float bf = std::min(std::max(1.5f - std::fabs(4.0f * value - 1.0f), 0.0f), 1.0f);
    r = static_cast<uint8_t>(rf * 255.0f);
    g = static_cast<uint8_t>(gf * 255.0f);
    b = static_cast<uint8_t>(bf * 255.0f);
}

static bool point_in_crop(const Eigen::Vector3f &p, const Eigen::Vector3f &crop_min, const Eigen::Vector3f &crop_max)
{
    for(int axis = 0; axis < 3; axis++)
    {
        if(std::isfinite(crop_min(axis)) && p(axis) < crop_min(axis))
            return false;
        if(std::isfinite(crop_max(axis)) && p(axis) > crop_max(axis))
            return false;
    }
    return true;
}

static bool point_in_polygon_xy(const Eigen::Vector3f &p, const std::vector<Eigen::Vector2f> &polygon)
{
    if(polygon.size() < 3)
        return true;
...
static bool point_near_trajectory_xy(
    const Eigen::Vector3f &p,
    const std::vector<Eigen::Vector2f> &trajectory_xy,
    float keep_radius)
{
    if(trajectory_xy.empty() || keep_radius <= 0.0f)
        return true;

    const float keep_radius_sq = keep_radius * keep_radius;
    const Eigen::Vector2f point_xy(p(0), p(1));
    for(const auto &traj_xy : trajectory_xy)
    {
        if((point_xy - traj_xy).squaredNorm() <= keep_radius_sq)
            return true;
    }
    return false;
}
...
static VoxelKey point_to_voxel(const Eigen::Vector3f &p, float voxel_size)
{
    return VoxelKey{
...
static std::vector<Eigen::Vector3f> filter_sparse_voxels(
    const std::vector<Eigen::Vector3f> &points,
    float voxel_size,
    int min_neighbors)
{
    if(points.empty() || voxel_size <= 0.0f || min_neighbors <= 0)
        return points;

    std::unordered_map<VoxelKey, int, VoxelKeyHash> voxel_counts;
    voxel_counts.reserve(points.size());
    for(const auto &p : points)
        voxel_counts[point_to_voxel(p, voxel_size)]++;

    std::vector<Eigen::Vector3f> filtered;
    filtered.reserve(points.size());
    for(const auto &p : points)
    {
        const VoxelKey center = point_to_voxel(p, voxel_size);
...
static std::vector<Eigen::Vector3f> filter_low_height_outliers(
    const std::vector<Eigen::Vector3f> &points,
    int axis,
    float percentile,
    float margin)
{
    ...
    percentile = std::max(0.0f, std::min(0.5f, percentile));
    std::vector<float> heights;
    heights.reserve(points.size());
    for(const auto &p : points)
        heights.push_back(p(axis));
    std::sort(heights.begin(), heights.end());
    size_t percentile_index = static_cast<size_t>(percentile * static_cast<float>(heights.size() - 1));
    const float min_height = heights[percentile_index] + std::max(0.0f, margin);

    std::vector<Eigen::Vector3f> filtered;
    filtered.reserve(points.size());
    for(const auto &p : points)
    {
        if(p(axis) >= min_height)
            filtered.push_back(p);
    }
    return filtered;
}

void SurfelMap::save_height_cloud(
    const string &save_path_name,
    const string &height_axis,
    int min_update_times,
    const HeightCloudFilterOptions &filter_options)
{
    printf("saving height-colored pointcloud ...\n");
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointcloud(new pcl::PointCloud<pcl::PointXYZRGB>);
    std::vector<Eigen::Vector3f> points;
    points.reserve(local_surfels.size() + inactive_pointcloud->size());

    for(int surfel_it = 0; surfel_it < local_surfels.size(); surfel_it++)
    {
        if(local_surfels[surfel_it].update_times < min_update_times)
            continue;
        Eigen::Vector3f p(local_surfels[surfel_it].px, local_surfels[surfel_it].py, local_surfels[surfel_it].pz);
        if(point_in_crop(p, filter_options.crop_min, filter_options.crop_max) &&
           point_in_polygon_xy(p, filter_options.crop_polygon) &&
           point_near_trajectory_xy(p, filter_options.trajectory_xy, filter_options.trajectory_keep_radius))
            points.push_back(p);
    }
    for(int point_it = 0; point_it < inactive_pointcloud->size(); point_it++)
    {
        const PointType &p = inactive_pointcloud->points[point_it];
        Eigen::Vector3f point(p.x, p.y, p.z);
        if(point_in_crop(point, filter_options.crop_min, filter_options.crop_max) &&
           point_in_polygon_xy(point, filter_options.crop_polygon) &&
           point_near_trajectory_xy(point, filter_options.trajectory_xy, filter_options.trajectory_keep_radius))
            points.push_back(point);
    }

    const size_t before_density_filter = points.size();
    points = filter_sparse_voxels(
        points,
        filter_options.density_voxel_size,
        filter_options.density_min_neighbors);

    if(points.empty())
    {
        ROS_WARN("no surfels to save");
        pcl::io::savePCDFileBinary(save_path_name.c_str(), *pointcloud);
        return;
    }

    int axis = height_axis_index(height_axis);
    if(filter_options.remove_low_height_outliers)
        points = filter_low_height_outliers(
            points,
            axis,
            filter_options.low_height_percentile,
            filter_options.low_height_margin);

    if(points.empty())
    {
        ROS_WARN("all surfels were removed by height filtering");
        pcl::io::savePCDFileBinary(save_path_name.c_str(), *pointcloud);
        return;
    }

    float h_min = points.front()(axis);
    float h_max = h_min;
    for(const auto &p : points)
    {
        h_min = std::min(h_min, p(axis));
        h_max = std::max(h_max, p(axis));
    }
    float h_range = std::max(h_max - h_min, 1e-6f);
    ...
    for(const auto &p_in : points)
    {
        ...
        p.z = p_in(2);
        uint8_t r, g, b;
        jet_color((p_in(axis) - h_min) / h_range, r, g, b);
        p.r = r;
        p.g = g;
        p.b = b;
        pointcloud->push_back(p);
    ...
    pcl::io::savePCDFileBinary(save_path_name.c_str(), *pointcloud);
    printf("height color axis=%s, min=%f, max=%f\n", height_axis.c_str(), h_min, h_max);
    printf("auto filters: trajectory_radius=%f, density_voxel=%f, density_min_neighbors=%d, before_density=%lu\n",
           filter_options.trajectory_keep_radius,
           filter_options.density_voxel_size,
           filter_options.density_min_neighbors,
           before_density_filter);
    printf("saved %s with %lu points\n", save_path_name.c_str(), pointcloud->size());
}
