...
struct HeightCloudFilterOptions
{
    Eigen::Vector3f crop_min;
    Eigen::Vector3f crop_max;
    std::vector<Eigen::Vector2f> crop_polygon;
    std::vector<Eigen::Vector2f> trajectory_xy;
    float trajectory_keep_radius;
    float density_voxel_size;
    int density_min_neighbors;
    bool remove_low_height_outliers;
    float low_height_percentile;
    float low_height_margin;

    HeightCloudFilterOptions();
};
...
public:
    ...
    void process_offline_frame(const ros::Time &stamp, cv::Mat &depth, Eigen::Matrix4f &pose);
    void save_height_cloud(
