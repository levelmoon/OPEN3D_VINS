...
PoseGraph::PoseGraph()
{
    ...
    use_imu = 0;
    loop_mode = LOOP_AUTO;
    manual_prior_valid = false;
    manual_prior_x = 0.0;
    manual_prior_y = 0.0;
    manual_prior_yaw_deg = 0.0;
    manual_prior_radius = 2.0;
    manual_prior_yaw_radius_deg = 45.0;
    manual_fail_count = 0;
    hybrid_fallback_fail_threshold = 10;
}
...
void PoseGraph::loadVocabulary(std::string voc_path)
{
    ...
    manual_db.setVocabulary(*voc, false, 0);
}
...
{
    if (loop_mode == LOOP_AUTO)
        return detectLoopAuto(keyframe, frame_index);

    if (loop_mode == LOOP_MANUAL_PRIOR)
    {
        int manual_result = detectLoopManual(keyframe, frame_index);
        db.add(keyframe->brief_descriptors);
        return manual_result;
    }

    if (loop_mode == LOOP_HYBRID)
    {
        int manual_result = detectLoopManual(keyframe, frame_index);
        if (manual_result != -1)
        {
        ...
        manual_fail_count++;
        if (manual_fail_count >= hybrid_fallback_fail_threshold)
        {
            int auto_result = detectLoopAuto(keyframe, frame_index);
            if (auto_result != -1)
                manual_fail_count = 0;
            return auto_result;
        }
...
int PoseGraph::detectLoopAuto(KeyFrame* keyframe, int frame_index)
{
...
int PoseGraph::detectLoopManual(KeyFrame* keyframe, int frame_index)
{
    if (!manual_prior_valid || manual_db_to_keyframe_index.empty())
        return -1;

    QueryResults ret;
    manual_db.query(keyframe->brief_descriptors, ret, 4, -1);
    ...
    if (find_loop)
    {
        ...
        for (unsigned int i = 0; i < ret.size(); i++)
        {
            ...
            int db_id = ret[i].Id;
            if (db_id >= 0 && db_id < (int)manual_db_to_keyframe_index.size() && ret[i].Score > best_score)
            {
                best_score = ret[i].Score;
                best_index = manual_db_to_keyframe_index[db_id];
            }
        }

        if (best_index != -1)
        {
            printf("[loop_fusion] manual prior loop candidate: current=%d old=%d score=%.4f\n",
                   frame_index, best_index, best_score);
            return best_index;
        }
    }
...
void PoseGraph::setLoopMode(int mode)
{
    if (mode == 0)
        loop_mode = LOOP_AUTO;
    else if (mode == 1)
        loop_mode = LOOP_MANUAL_PRIOR;
    else if (mode == 2)
        loop_mode = LOOP_HYBRID;
    else
        loop_mode = LOOP_AUTO;

    manual_fail_count = 0;
    printf("[loop_fusion] loop mode set to %d\n", loop_mode);
}

void PoseGraph::setManualPrior(double x, double y, double yaw_deg, double radius, double yaw_radius_deg)
{
    manual_prior_x = x;
    manual_prior_y = y;
    manual_prior_yaw_deg = yaw_deg;
    manual_prior_radius = radius;
    manual_prior_yaw_radius_deg = yaw_radius_deg;
    manual_prior_valid = true;
    manual_fail_count = 0;

    printf("[loop_fusion] manual prior: x=%.3f y=%.3f yaw=%.3f radius=%.3f yaw_radius=%.3f\n",
           manual_prior_x, manual_prior_y, manual_prior_yaw_deg,
           manual_prior_radius, manual_prior_yaw_radius_deg);
}

double PoseGraph::normalizeAngleDeg(double angle)
{
    while (angle > 180.0)
        angle -= 360.0;
    while (angle < -180.0)
...
bool PoseGraph::isKeyFrameNearManualPrior(KeyFrame* keyframe)
{
    if (!manual_prior_valid)
        return false;
    ...
    double dx = P.x() - manual_prior_x;
    double dy = P.y() - manual_prior_y;
    double dist = sqrt(dx * dx + dy * dy);
    if (dist > manual_prior_radius)
        return false;

    double yaw = Utility::R2ypr(R).x();
    double yaw_error = fabs(normalizeAngleDeg(yaw - manual_prior_yaw_deg));
    if (yaw_error > manual_prior_yaw_radius_deg)
        return false;

    return true;
}

void PoseGraph::buildManualCandidateDatabase()
{
    manual_db.clear();
    manual_db_to_keyframe_index.clear();
    ...
    {
        KeyFrame* kf = *it;
        if (isKeyFrameNearManualPrior(kf))
        {
            manual_db.add(kf->brief_descriptors);
            manual_db_to_keyframe_index.push_back(kf->index);
            candidate_count++;
        }
    }
    m_keyframelist.unlock();

    printf("[loop_fusion] manual candidate keyframes: %d\n", candidate_count);
    if (candidate_count == 0)
        printf("[loop_fusion] warning: no manual candidates, increase radius/yaw_radius or use auto mode\n");
}
