...
static void pubKeyframeByIndex(const Estimator &estimator, int target_index, bool force_publish)
{
    // pub camera pose, 2D-3D points of keyframe
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR &&
        (force_publish || estimator.marginalization_flag == 0))
    {
        int i = target_index;
        ...
        if (force_publish)
            ROS_WARN("[vins_estimator] forced keyframe published at index %d, stamp %.6f, points %lu",
                     i, estimator.Headers[i], point_cloud.points.size());
    }
}

void pubKeyframe(const Estimator &estimator)
{
    pubKeyframeByIndex(estimator, WINDOW_SIZE - 2, false);
}

void pubForcedKeyframe(const Estimator &estimator)
{
    pubKeyframeByIndex(estimator, WINDOW_SIZE - 1, true);
}
