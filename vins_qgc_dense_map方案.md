...
rosbag
  |
  | raw depth + depth camera info + VINS 轨迹
  v
DenseSurfelMapping offline_surfel_from_bag
  |
  | depth-only surfel fusion + height color
  v
map.pcd
```

VINS-Fusion 本身不是稠密建图算法，它主要提供高质量 IMU/body 位姿、回环和位姿图优化。当前稠密地图由 DenseSurfelMapping 的 `offline_surfel_from_bag` 基于 raw depth、depth camera info 和 VINS 轨迹离线融合生成。
...
保存 VINS-Fusion 回环优化后的轨迹，当前建图程序读取格式：
...
### 4.3 使用 DenseSurfelMapping 生成 PCD

该方案只使用 raw depth、depth camera info 和 VINS IMU/body 轨迹。程序会将 VINS 位姿通过 `T_body_depth` 转换到 depth camera 位姿，并把每帧 depth 融合为 surfel 地图，最后输出高度着色的 PCL 兼容 `PCD` 文件。
...
YAML 中的相对路径会按 `surfel_fusion` 包目录解析。按当前 Jetson 工作区结构，bag、轨迹和输出文件可以写成 `../../../levelmoon/vio_loop.txt` 这种路径。

YAML 中 `shutdown_roslaunch_on_finish: true` 表示离线建图成功保存 PCD 后自动结束 `roslaunch`，避免命令挂在终端里。

如果 VINS 配置文件里有 `body_T_depth`，程序会直接使用它；否则会读取 `body_T_cam0` 和 `body_T_cam1`，取双目中心，再叠加 `--stereo-depth-tx/ty/tz` 得到 depth camera 外参。

推荐调参：

```text
max-pose-dt: 0.05 - 0.10 s
max-angular-rate: 0.3 - 0.6 rad/s
trajectory-keep-radius: 2.0 - 3.0 m
density-voxel-size: 0.15 - 0.30 m
density-min-neighbors: 3 - 6
```

如果点云太稀：

```bash
--density-min-neighbors 2 --min-update-times 3
```

如果噪声明显：

```bash
--max-angular-rate 0.3 --density-min-neighbors 5 --trajectory-keep-radius 2.0
```
...
3. DenseSurfelMapping depth-only 融合 raw depth。
4. 导出 map.pcd。
5. 保存 map_meta.yaml。
```
...
- DenseSurfelMapping 离线融合生成高度着色 PCD 地图。
- QGC 负责任务调度。
- 地面站地图程序负责稠密点云可视化。
